    ...
    <header class="glass fixed top-0 left-0 z-50 w-full border-b border-white/10 bg-black/20 backdrop-blur-md transition-all duration-300">
      <div class="container mx-auto flex items-center justify-between px-6 py-4">
        ...
        <nav class="hidden items-center space-x-8 md:flex">
          <Link
            href="/"
            class={`border-b-2 pb-1 font-medium text-gray-300 transition-colors hover:text-cyan-400 ${location.url.pathname === "/" ? "border-cyan-400 text-cyan-400" : "border-transparent"}`}
          >
            Home
          </Link>
          <Link
            href="/reflection"
            class={`border-b-2 pb-1 font-medium text-gray-300 transition-colors hover:text-cyan-400 ${location.url.pathname.startsWith("/reflection") ? "border-cyan-400 text-cyan-400" : "border-transparent"}`}
          >
            Reflections
          </Link>
          <a
